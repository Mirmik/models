...
base = (
	...
	#+ box()
	#+ cylinder(r=)
	#+ scaletrans(cone(r1=re1, r2=re2, h=h, angle=deg(90)))
...
holder = polyg.extrude(hold_t).fillet(hold_fil2)
holder = holder + mirrorYZ()(holder)
...
holder = holder + det
holder = holder.rotateY(deg(90)).up(polyg_xseg*4).left(hold_t/2).forw(re2 - polyg_yseg*0.4 + 1.5)
